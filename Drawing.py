...
def draw_graph(vertices, aristas, MIN_WIDTH, MIN_HEIGHT, MAX_WIDTH, MAX_HEIGHT):
    ...
    for dummy_index in range(len(vertices)):
        ...
        plt.text(vertices[dummy_index][0]+0.1, vertices[dummy_index][1]+0.1, str(dummy_index), fontsize=8)
        ...
        if (vertices[dummy_index][2] != 0 and dummy_index < 10):
        	plt.text(vertices[dummy_index][0], vertices[dummy_index][1], "    "+str(vertices[dummy_index][2]), fontsize=4)
    ...
    for dummy_index in range(len(edges)):
        ...
        if(len("["+str(edges[dummy_index][0])+","+str(edges[dummy_index][1])+"]") >= 6):
            correct_position = label_correction([mid_point[0]-0.5,mid_point[1]], labels_positions)
            plt.text(correct_position[0], correct_position[1], "     "+str(edges[dummy_index][2]), color= "brown", fontsize=7)
            plt.text(correct_position[0], correct_position[1]-0.1, "["+str(edges[dummy_index][0])+","+str(edges[dummy_index][1])+"]" , color= "brown", fontsize=4)
        ...
        if(edges[dummy_index][3] >= 10):
            ...
            plt.text(correct_position[0], correct_position[1], "        "+str(edges[dummy_index][3]), color= "red", fontsize=7)
            plt.text(correct_position[0], correct_position[1], "                       ["+str(edges[dummy_index][0])+","+str(edges[dummy_index][1])+"]" , color= "red", fontsize=4)
            labels_positions.append([correct_position[0], correct_position[1]])
        else:
            correct_position = label_correction([mid_point[0]+0.25,mid_point[1]], labels_positions)
            plt.text(correct_position[0], correct_position[1], "       "+str(edges[dummy_index][3]), color= "red", fontsize=7)
            plt.text(correct_position[0], correct_position[1]-0.1, "                     ["+str(edges[dummy_index][0])+","+str(edges[dummy_index][1])+"]" , color= "red", fontsize=4)
            labels_positions.append([correct_position[0], correct_position[1]])
    ...
    plt.savefig('Instance.pdf', format='pdf')
